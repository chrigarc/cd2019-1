    ...
    public void run(){
        ...
        while (activo) {
            ...
            while(iterator.hasNext()){
                ...
                Message newMessage = new Message(node.getId(), node.getId());
                this.sendMessage(newMessage, n.getId());
            }
            sleep(300);
            Message myMessage = this.readMessage();
            if(myMessage != null){
                myMessage.leerMessage();
                iterator = node.getNeighborNodeIterator();
                while(iterator.hasNext()){
                    Node n = iterator.next();
                    this.sendMessage(myMessage.clone(), n.getId());
                }
            }
        }
    }
